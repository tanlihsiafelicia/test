...
def get_attr_name(light_selection_list, light_types):
...
def set_attribute(loaded_data):
	...
	for key, value in loaded_data.items():
		...
		# except Exception as e:
		# 	print(f"Failed to set {key} with value {value}: {e}")
	...
	# 	for key2, value2 in attribute_type.items():
	# 		if len(value) == value2:
	# 				attr_type = key2
	# 				# break
...
def main_export():
	...
	attributes_shape 		= get_attr_name(light_shapes, light_types)[0]
	# pp.pprint(attributes_shape)
	attributes_transform 	= get_attr_name(light_transform, light_types)[1]
	# pp.pprint(attributes_transform)
	attributes_group 		= get_attr_name(light_group, light_types)[2]
	# print(attributes_group)
	...
	# new_dict[light_shapes] = shape_dict
	# new_dict[light_transform] = transform_dict
	# new_dict[light_group] = group_dict
