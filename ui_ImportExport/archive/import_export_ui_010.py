	...
	for key, value in combined_dict['attribute_dict'].items():
		...
		for key2, value2 in value.items():
			...
			if key2 == 'parent':
				...
				for v in value2:
					# print(value2)
					cleaned_key2 = v.split(':')[-1]
					value2.append(cleaned_key2)
				print(value2)
				# continue
	...
	for key, value in combined_dict['attribute_dict'].items():
		...
		for key2, value2 in value.items():
			...
			# Exclude 'pivot_data' data from setAttr
				...
				namespaced_key2[key2_wth_namespace] = value2 # Add into dict

		temp_dict[key_wth_namespace] = namespaced_key2
		namespaced_key.update(temp_dict)

	combined_dict_namespaced['attribute_dict'] = namespaced_key
	# pp.pprint(combined_dict_namespaced)
	return combined_dict_namespaced
...
def main_export():
	...
	# creating_json = create_json(removing_namespace)
