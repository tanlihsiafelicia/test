...
def get_curve_data(curve_node):
	...
	if custom_attr:
		...
		for attr in custom_attr:
			# print(attr)
			curve_value = cmds.getAttr(f'{curve_node}.{attr}') # Gets custom_attr values
			# print(f'{curve_node}.{attr}: {curve_value}')
		...
		custom_attr = []  # Ensure it's an empty list
		curve_data[(f'{curve_node}.custom_attr')] = {}  # Store an empty dictionary for this node
...
def get_custom_attr_setting(curve_node):
	...
	# curve_node = cmds.ls(type='nurbsCurve')[0]
	
	# Check userDefined custom attributes
	custom_attr = cmds.listAttr(curve_node, userDefined=True)
	# print(f'{curve_node}.{custom_attr}')
	...
	if custom_attr:
		for attr in custom_attr:
			...
			# print(f'{curve_node}{attr}: {custom_attr_type}')
			...
			if custom_attr_type == 'double': # float
				...
				# curve_value_float_list.append(curve_value) # Append curve attr value 
				...
				# print(f'{curve_node}.{attr}: {curve_value}')
				# print(f'{curve_node}.{attr}.min_val: {min_val}')
				# print(f'{curve_node}.{attr}.max_val: {max_val}')
				# print(f'{curve_node}.{attr}.def_val: {def_val}')
				
				# Create dict for custom attributes settings
				custom_setting_dict[f'{curve_node}.{attr}'] = curve_value_float_list
			...
			if custom_attr_type == 'long': # integer
				...
				# curve_value_int_list.append(curve_value) # Append curve attr value 
				curve_value_int_list.append(min_val[0]) # Append min value
				curve_value_int_list.append(max_val[0]) # Append max value
				curve_value_int_list.append(def_val[0]) # Append default value
				# print(f'{curve_node}.{attr}: {curve_value}')
				# print(f'{curve_node}.{attr}.min_val: {min_val}')
				# print(f'{curve_node}.{attr}.max_val: {max_val}')
				# print(f'{curve_node}.{attr}.def_val: {def_val}')
				...
				# Create dict for custom attributes settings
				custom_setting_dict[f'{curve_node}.{attr}'] = curve_value_int_list

			if custom_attr_type == 'enum': # enum
				# if cmds.attributeQuery(attr, node=curve_node, attributeType= True) == 'enum':
				enum_names = cmds.attributeQuery(attr, node=curve_node, listEnum= True)
				enum_list = enum_names[0].split(':')

				# Create dict for custom attributes settings
				custom_setting_dict[f'{curve_node}.{attr}'] = enum_list
...
def import_animExport(read_json_file, animExport_file_path):
...
# 	import_anim = import_animExport(read_json_file, animExport_file_path)
