...
def create_light(loaded_data, lgt_key):
...
def create_curve(loaded_data, lgt_key):
...
def create_custom_attr(loaded_data, lgt_key):
...
def set_pivot(loaded_data, lgt_key):
...
def set_attribute(loaded_data, lgt_key):
	...
	if insert_namespace == '': # WITHOUT namespace
		...
		for key, value in read_json_file['attribute_dict'].items():
			...
			if not lgtrig_exist: # If light rig NOT EXIST in scene
				...
				creating_create_light	= create_light(read_json_file, key)
				creating_create_curve	= create_curve(read_json_file, key)
				creating_custom_attr	= create_custom_attr(read_json_file, key)
				setting_pivot			= set_pivot(read_json_file, key)
		...
		# 	if lgtrig_exist: # If light rig NOW EXIST in scene
				...
				seting_attr 			= set_attribute(read_json_file, key)
		...
		for key, value in added_namespace['attribute_dict'].items():
			...
			if namespace_lgtrig_exist: # If namespace light rig NOT EXIST in scene
				print('light EXIST with namespace')
				set_attr 		= set_attribute(added_namespace, key)
				import_anim 	= import_animExport(animExport_file_path)
